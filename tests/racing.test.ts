...
import { newRaceState, stepRace, type RaceState } from '../src/colony/racing/race'
...
function seededTrafficTown(): ColonySim {
  ...
  sim.state.buildings.push({ id: 9001, x: a.x, y: a.y - 1, artifact: art(9001, 'habitat') })
  sim.state.buildings.push({ id: 9002, x: b.x, y: b.y + 1, artifact: art(9002, 'commercial') })
...
function manualTrack(): RaceTrack {
  return {
    checkpoints: [{ x: 1, y: 1 }, { x: 2, y: 1 }, { x: 3, y: 1 }, { x: 4, y: 1 }],
    path: [{ x: 1, y: 1 }, { x: 2, y: 1 }, { x: 3, y: 1 }, { x: 4, y: 1 }],
    length: 3,
    loop: false,
    seed: 1,
    roadsVersion: 1,
    roadKinds: { '1,1': 'street', '2,1': 'street', '3,1': 'street', '4,1': 'street' },
  }
}

function running(track: RaceTrack, x: number, y: number): RaceState {
  const r = newRaceState(track)
  return { ...r, mode: 'running', countdownMs: 0, nextCheckpoint: track.checkpoints.length, car: { ...r.car, x, y, speed: 0 } }
}
...
describe('spec 087 road rally track generation', () => {
  ...
  it('puts checkpoints and the full route only on dry roadKind cells', () => {
    for (const seed of SEEDS) {
      ...
      for (const cp of track.checkpoints) {
        expect(rt.sim.state.roadKind.has(roadKey(cp.x, cp.y)), `checkpoint ${cp.x},${cp.y}`).toBe(true)
        expect(rt.sim.state.terrain.isWater(cp.x, cp.y), `checkpoint water ${cp.x},${cp.y}`).toBe(false)
      }
      for (const p of track.path) {
        expect(rt.sim.state.roadKind.has(roadKey(p.x, p.y)), `path ${p.x},${p.y}`).toBe(true)
        expect(rt.sim.state.terrain.isWater(p.x, p.y), `path water ${p.x},${p.y}`).toBe(false)
      }
    }
  }, 30000)

  it('has a first-class null-commercial out-and-back fallback on a minimal road line', () => {
    const track = makeRaceTrack(tinyRoadState(), { commercialCenter: null, seed: 7 })
    expect(track).not.toBeNull()
  ...
  it('keeps the start checkpoint out of the Nearest bar footprint and frontage', () => {
    ...
    for (const cp of track.checkpoints) expect(excluded.has(roadKey(cp.x, cp.y))).toBe(false)
  }, 30000)
  ...
  it('honors the numeric contracts on live seeds', () => {
    for (const seed of SEEDS) {
      ...
      if (center) expect(Math.hypot(track.checkpoints[0]!.x - center.x, track.checkpoints[0]!.y - center.y)).toBeLessThanOrEqual(COMMERCIAL_START_MAX_DIST)
    }
  }, 30000)
})
