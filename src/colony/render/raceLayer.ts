...
export function buildRaceLayer(opts: RaceLayerOptions): RaceLayer | null {
  if (opts.track.path.length < 2 || opts.track.checkpoints.length < 2) return null
  ...
  return {
    ...
    update(raceState: RaceState, timeMs: number) {
      ...
      for (let i = 0; i < checkpointMats.length; i++) {
        const active = i === raceState.nextCheckpoint || (raceState.track.loop && raceState.nextCheckpoint >= raceState.checkpoints.length && i === 0)
        checkpointMats[i]!.emissiveIntensity = active ? 0.48 + pulse * 0.16 : 0.24
      }
    },
    dispose() {
      group.traverse((obj) => {
...
function buildPlayerCar(): THREE.Group {
  ...
  const bodyMat = new THREE.MeshStandardMaterial({ color: 0xff4c52, roughness: 0.5, metalness: 0.12, emissive: 0x451010, emissiveIntensity: 0.35 })
  const cabinMat = new THREE.MeshStandardMaterial({ color: 0xffd24d, roughness: 0.42, metalness: 0.08, emissive: 0x3a2a08, emissiveIntensity: 0.28 })
  const wheelMat = new THREE.MeshStandardMaterial({ color: 0x171820, roughness: 0.72, metalness: 0.04 })
  const body = new THREE.Mesh(new THREE.BoxGeometry(1.18, 0.34, 0.68), bodyMat)
  body.position.y = 0.22
  const cabin = new THREE.Mesh(new THREE.BoxGeometry(0.48, 0.28, 0.46), cabinMat)
  cabin.position.set(-0.1, 0.52, 0)
  g.add(body, cabin)
  for (const x of [-0.42, 0.42]) {
    for (const z of [-0.38, 0.38]) {
      const wheel = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 0.12, 10), wheelMat)
...
function buildCheckpoint(opts: RaceLayerOptions, idx: number, mats: THREE.MeshStandardMaterial[]): THREE.Group {
  ...
  const mat = new THREE.MeshStandardMaterial({ color: idx === 0 ? 0x66e0ff : 0xffd75c, emissive: idx === 0 ? 0x2aa8d8 : 0xd89a2a, emissiveIntensity: 0.36, roughness: 0.48, metalness: 0.04 })
  mats.push(mat)
  const postGeo = new THREE.CylinderGeometry(0.07, 0.08, 1.45, 8)
  const barGeo = new THREE.BoxGeometry(1.55, 0.12, 0.12)
  for (const s of [-1, 1]) {
    const post = new THREE.Mesh(postGeo, mat)
    post.position.set(opts.wx(cp.x + side.x * s * 0.72), baseY + 0.72, opts.wz(cp.y + side.y * s * 0.72))
    g.add(post)
  }
...
function checkpointTangent(track: RaceTrack, cp: { x: number; y: number }): { x: number; y: number } {
  let best = 0
  let bestD = Infinity
  ...
  const b = track.path[Math.min(track.path.length - 1, best + 1)] ?? track.path[best]!
  const dx = b.x - a.x
  const dy = b.y - a.y
