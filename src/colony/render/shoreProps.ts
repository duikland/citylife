...
export function buildShoreProps(opts: ShorePropsOptions): ShorePropsLayer | null {
  ...
  tower.position.set(opts.wx(lighthouse.x), baseY + 0.03, opts.wz(lighthouse.y))
  group.add(tower)
  ...
  return {
    ...
    update(daylight: number, timeMs: number) {
      ...
      for (const mat of lanternMats) mat.emissiveIntensity = 0.4 + night * 0.78 + pulse * 0.22
    },
    dispose() {
      group.traverse((obj) => {
...
function buildLighthouse(lanternMats: THREE.MeshStandardMaterial[]): THREE.Group {
  const g = new THREE.Group()
  const towerMat = new THREE.MeshStandardMaterial({ color: 0xf1ead8, roughness: 0.72, metalness: 0.02 })
  const redMat = new THREE.MeshStandardMaterial({ color: 0xb73a32, roughness: 0.6, metalness: 0.03 })
  const stoneMat = new THREE.MeshStandardMaterial({ color: 0x6c6870, roughness: 0.95, metalness: 0.02, flatShading: true })
  const darkMat = new THREE.MeshStandardMaterial({ color: 0x24242a, roughness: 0.86 })
  ...
  const base = new THREE.Mesh(new THREE.CylinderGeometry(1.18, 1.32, 0.42, 12), stoneMat)
  base.position.y = 0.21
  const tower = new THREE.Mesh(new THREE.CylinderGeometry(0.5, 0.88, 6.8, 16), towerMat)
  tower.position.y = 3.64
  const lowerBand = new THREE.Mesh(new THREE.CylinderGeometry(0.78, 0.84, 0.14, 16), redMat)
  lowerBand.position.y = 2.55
  const redBand = new THREE.Mesh(new THREE.CylinderGeometry(0.64, 0.7, 0.3, 16), redMat)
  redBand.position.y = 7.05
  const lantern = new THREE.Mesh(new THREE.CylinderGeometry(0.62, 0.62, 0.74, 16), lanternMat)
  lantern.position.y = 7.56
  const lamp = new THREE.Mesh(new THREE.SphereGeometry(0.28, 12, 8), lanternMat)
  lamp.position.y = 7.56
  const glow = new THREE.Mesh(
    new THREE.SphereGeometry(0.56, 12, 8),
    new THREE.MeshBasicMaterial({ color: 0xffd78b, transparent: true, opacity: 0.18 }),
  )
  glow.position.y = 7.56
  const cap = new THREE.Mesh(new THREE.ConeGeometry(0.82, 0.92, 16), redMat)
  cap.position.y = 8.42
  const rail = new THREE.Mesh(new THREE.TorusGeometry(0.86, 0.03, 6, 20), darkMat)
...
function addRockeryBeach(group: THREE.Group, opts: ShorePropsOptions, lighthouse: SeedStructure): void {
  ...
  const mark = (x: number, y: number, r: number) => {
    for (let yy = y - r; yy <= y + r; yy++) for (let xx = x - r; xx <= x + r; xx++) blocked.add(`${xx},${yy}`)
  }
  for (const st of opts.structures) mark(st.x, st.y, st.kind === 'lighthouse' ? 2 : 1)

  const candidates: { x: number; y: number; h: number }[] = []
  for (let y = Math.max(1, lighthouse.y - ROCKERY_RADIUS); y <= Math.min(t.size - 2, lighthouse.y + ROCKERY_RADIUS); y++) {
    for (let x = Math.max(1, lighthouse.x - ROCKERY_RADIUS); x <= Math.min(t.size - 2, lighthouse.x + ROCKERY_RADIUS); x++) {
  ...
  const rockMesh = new THREE.InstancedMesh(
    ...
    new THREE.MeshStandardMaterial({ roughness: 0.94, metalness: 0.02, flatShading: true, vertexColors: true }),
    PROP_CAP,
  )
  const sitMesh = new THREE.InstancedMesh(
    new THREE.DodecahedronGeometry(0.72, 0),
    new THREE.MeshStandardMaterial({ color: 0x8b8792, roughness: 0.96, metalness: 0.02, flatShading: true }),
    PROP_CAP,
  )
  const driftMesh = new THREE.InstancedMesh(
    new THREE.BoxGeometry(1, 0.12, 0.18),
    new THREE.MeshStandardMaterial({ color: 0x7a5638, roughness: 0.9, metalness: 0.01 }),
    PROP_CAP,
  )
  const grassMesh = new THREE.InstancedMesh(
    new THREE.ConeGeometry(0.12, 0.46, 5),
    new THREE.MeshStandardMaterial({ color: 0x6f8f58, roughness: 0.88, metalness: 0, flatShading: true }),
    PROP_CAP,
  )
  let rocks = 0, sits = 0, drifts = 0, grasses = 0
...
function shorePropCellOk(t: Terrain, x: number, y: number, blocked: Set<string>): boolean {
...
function hashCell(x: number, y: number, salt: number): number {
  let h = (Math.imul(x + 0x9e37, 374761393) ^ Math.imul(y + salt, 668265263)) >>> 0
