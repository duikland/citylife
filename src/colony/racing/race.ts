...
export function newRaceState(track: RaceTrack): RaceState {
  ...
  const next = track.path.find((p) => p.x !== start.x || p.y !== start.y) ?? track.checkpoints[1] ?? { x: start.x + 1, y: start.y }
  return {
    mode: 'countdown',
    track,
    car: {
    ...
    checkpoints: track.checkpoints.map((p, i) => ({ x: p.x, y: p.y, crossed: i === 0 })),
...
export function stepRace(state: RaceState, input: RaceInput, dtMs: number): RaceState {
...
export function crossedCheckpoint(state: RaceState, idx: number): boolean {
  ...
  if (!cp) return false
  return Math.hypot(state.car.x - cp.x, state.car.y - cp.y) <= CHECKPOINT_RADIUS
}
...
function driveCar(state: RaceState, input: RaceInput, dt: number): void {
  ...
  const cell = state.track.path[Math.max(0, Math.min(state.track.path.length - 1, before.pathIndex))] ?? state.track.path[0]
  const kind = cell ? state.track.roadKinds[`${cell.x},${cell.y}`] ?? 'street' : 'street'
  const onTrack = before.distance <= 0.9
  const maxForward = (kind === 'avenue' ? 8.8 : kind === 'street' ? 7.2 : 4.8) * (onTrack ? 1 : 0.48)
  ...
  if (steer !== 0 && Math.abs(car.speed) > 0.05) {
    ...
    car.heading += steer * dir * (2.25 + Math.min(5, Math.abs(car.speed)) * 0.12) * handbrake * dt
  }
...
function advanceCheckpoint(state: RaceState): void {
  ...
  if (state.nextCheckpoint < state.checkpoints.length) {
    ...
    if (state.nextCheckpoint >= state.checkpoints.length && !state.track.loop) finish(state)
    return
  }
