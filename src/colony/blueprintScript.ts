...
export const ROOM_KINDS: readonly RoomKind[] = ['living', 'bedroom', 'garage', 'patio', 'pool']
...
function parseFields(body: string): Record<string, string> {
  ...
  for (const tok of body.trim().split(/\s+/).filter(Boolean)) {
    const i = tok.indexOf(':')
    if (i <= 0) throw new Error(`blueprint: bad field ${JSON.stringify(tok)} (expected key:value)`)
    out[tok.slice(0, i)] = tok.slice(i + 1)
  }
  return out
}

function intField(fields: Record<string, string>, key: string, ctx: string): number {
  const raw = fields[key]
  if (raw === undefined) throw new Error(`blueprint: ${ctx} is missing ${key}`)
  const n = Number(raw)
  if (!Number.isInteger(n)) throw new Error(`blueprint: ${ctx} ${key} must be an integer, got ${JSON.stringify(raw)}`)
  return n
}
...
export function parseBlueprint(script: BlueprintScript): ParsedBlueprint {
  if (typeof script !== 'string') throw new Error('blueprint: script must be a string')
  ...
  if (!DOOR_DIRS.includes(doorDir)) throw new Error(`blueprint: house door must be one of n s e w, got ${JSON.stringify(hf['door'])}`)
  ...
  while ((m = roomRe.exec(script)) !== null) {
    ...
    if (!ROOM_KINDS.includes(kind)) throw new Error(`blueprint: room kind must be one of ${ROOM_KINDS.join(' ')}, got ${JSON.stringify(rf['kind'])}`)
    rooms.push({
      kind,
...
export function blueprintToScript(p: ParsedBlueprint): BlueprintScript {
  ...
  const rooms = p.rooms.map((r) => `room{kind:${r.kind} x:${r.x} y:${r.y} w:${r.w} d:${r.d} win:${r.win ? 1 : 0}}`)
...
function roomTouchesDoorEdge(p: ParsedBlueprint): boolean {
  return p.rooms.some((r) => {
    switch (p.doorDir) {
      case 'n': return r.y === 0
      case 's': return r.y + r.d === p.d
      case 'w': return r.x === 0
      case 'e': return r.x + r.w === p.w
    }
  })
}
...
export function validateBlueprint(script: BlueprintScript): ValidationResult {
  ...
  try {
    ...
    return { ok: false, errors: [e instanceof Error ? e.message : String(e)], estMaterials: 0 }
  }
  const errors: string[] = []
  ...
  else if (!roomTouchesDoorEdge(p)) errors.push(`no room reaches the ${p.doorDir} door edge (door not reachable)`)
  return { ok: errors.length === 0, errors, estMaterials: estimateMaterials(p) }
}
