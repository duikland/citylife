...
import { dreamBrief, negotiate, priceBrief, briefToBlueprint, seededBudget, VIW_SEED, type Brief } from './negotiation'
...
export function BuilderDesk({ seed, zoneW, zoneD, onAccept }: {
  seed: number
  zoneW: number
  zoneD: number
  ...
  const dream = useMemo(() => dreamBrief(seed, { w: zoneW, d: zoneD }, 's'), [seed, zoneW, zoneD])
  ...
  return (
    <div data-build-area="builder-desk" style={{ ...panel, width: 280, display: 'flex', flexDirection: 'column', gap: 8, overflowY: 'auto' }}>
      ...
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', fontSize: 12 }}>
        ...
        <button data-build-action="budget-down" style={btn} onClick={() => setBudget((b) => Math.max(0, b - 25))}>−25</button>
        <b data-build-area="budget-value" style={{ minWidth: 56, textAlign: 'center' }}>{budget}</b>
        <button data-build-action="budget-up" style={btn} onClick={() => setBudget((b) => b + 25)}>+25</button>
      </div>

      <div data-build-area="negotiation" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      {session.state === 'agreed' ? (
        <div data-build-area="deal" style={{ borderTop: '1px solid #232c3f', paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={{ fontSize: 12 }}>🤝 <b>Agreed</b> — {briefLine(session.agreedBrief!)} for <b>{session.agreedPrice}</b> city coin.</div>
          <button
