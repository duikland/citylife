      ...
      <aside className="hud">
        ...
        {ui.neighborhood.lots.length > 0 && <div className="row" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 3 }}>{ui.neighborhood.lots.slice(0, 8).map((l) => {
          ...
          return <div key={l.id} style={{ display: 'flex', gap: 5, alignItems: 'center', fontSize: 11 }}>
            <span style={{ flex: 1, color: l.reserved ? '#e8a06a' : l.built ? '#cdbf9e' : l.owner ? '#c9a23a' : '#7a8a7a' }} title={l.reserved ? 'Founder plot — permanently reserved; never assigned to newcomers and protected from demolition.' : undefined}>{l.id.replace('lot_', 'Plot ')}{l.owner ? ` · ${l.reserved ? l.owner : l.owner.split(' ')[0]}` : ' · free'}{l.reserved ? ' 🦀 · Founder' : l.built ? ' 🏠' : ''}</span>
            {!l.ownerId && !l.reserved && firstFree && <button style={{ padding: '0 6px', fontSize: 10 }} onClick={() => runtime.assignLot(firstFree.id, l.id)} title={`Give this homestead to ${firstFree.displayName}`}>Assign</button>}
