...
import { Governor, type DecisionRecord, type ProviderName } from '../ai/governor'
...
export interface UiState {
  ...
  paused: boolean
  speed: number
  clock: { day: number; dayOfWeek: number; hour: number; minute: number; isDay: boolean }
  metrics: { population: number; treasury: number; happiness: number; gdp: number; employmentRate: number; unemployed: number }
  taxRates: { residential: number; commercial: number; industrial: number }
  budget: { transport: number; safety: number; health: number; environment: number }
  prices: Record<Commodity, number>
  history: { day: number; population: number; treasury: number; happiness: number; gdp: number }[]
  governor: {
    enabled: boolean
...
export class Runtime {
  ...
  private scheduleGovernor() {
    ...
    this.governorTimer = setInterval(() => void this.checkInNow(), this.intervalMs)
  }
  ...
  setGovernorIntervalMs(ms: number) {
    this.intervalMs = Math.max(CONFIG.governor.minIntervalMs, Math.min(24 * 60 * 60 * 1000, Math.round(ms)))
    this.scheduleGovernor()
    this.emit()
  }
  ...
  getUiState(): UiState {
    ...
    for (const c of Object.keys(s.market) as Commodity[]) prices[c] = Math.round(s.market[c].price * 10) / 10
    return {
      ...
      governor: {
        ...
        msUntilNext: this.nextCheckInAt ? Math.max(0, this.nextCheckInAt - Date.now()) : 0,
